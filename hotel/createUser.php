...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($secondName_err) && empty($email_err)
         && empty($phone_err) && empty($login_err) && empty($password_err)) {
...
<form class="modal-content animate" style="min-width: 270px;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="container" >
        <table class="bordery">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <button type="submit" class="btn btn-primary" value="Создать">Создать</button>
                    </td>
                    <td>
                            <button type="button" name="Cancel" class="btn btn-primary" value="Отменить" onclick="document.getElementById('id02').style.display='none'" >Отменить</button>
                                            </td>
                </tr>
            </tbody>
        </table>
